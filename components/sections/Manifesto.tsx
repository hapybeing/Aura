...
export default function Manifesto() {
  ...
  useEffect(() => {
    ...
    // ── Feed manifesto progress into the Zustand store (→ ShardField) ──────
    const progressST = ScrollTrigger.create({
      trigger: el,
      start: 'top 85%',
      end:   'bottom 15%',
      onUpdate: (self) => {
        // Smooth progress: 0 → 1 → 0 as section enters / exits
        const raw = self.progress
        // Ease in first 20%, ease out last 20%
        const eased =
          raw < 0.2
            ? raw / 0.2
            : raw > 0.8
    ...
    return () => {
      ...
      ScrollTrigger.getAll().forEach((st) => {
        if (st.vars.trigger === el ||
            st.vars.trigger === dividerRef.current ||
            st.vars.trigger === principlesRef.current) {
          st.kill()
        }
      })
    }
    ...
    >
      ...
      <div className="flex items-center gap-6 mb-20">
        ...
        <div
          style={{
            ...
            height: 1,
            background:
              'linear-gradient(90deg, rgba(26,86,255,0.35), transparent)',
          }}
        />
      </div>
      ...
      <div className="mb-16 md:mb-24">
        ...
        <KineticText
          ...
          style={{
            ...
            WebkitTextStroke: '1px rgba(232,232,240,0.28)',
            color: 'transparent',
        ...
        <KineticText
          ...
          style={{
            ...
            color: '#1A56FF',
            textShadow:
              '0 0 40px rgba(26,86,255,0.55), 0 0 120px rgba(26,86,255,0.20)',
            marginTop: '-0.06em',
          } as React.CSSProperties}
        >
          FROM ENTROPY.
      ...
      <div
        ...
        style={{
          height: 1,
          background:
            'linear-gradient(90deg, rgba(26,86,255,0.5) 0%, rgba(77,127,255,0.3) 50%, transparent 100%)',
          marginBottom: '4rem',
        }}
      />
      ...
      <div className="mb-20 max-w-2xl">
        <KineticText
          ...
          style={{ fontSize: 'clamp(1rem, 1.6vw, 1.35rem)' } as React.CSSProperties}
        >
          We are not designers. We are not developers. We are the people who
          sit in the space between mathematics and sensation — and refuse to
          leave until something impossible exists that did not before.
        </KineticText>
      </div>
      ...
      >
        {PRINCIPLES.map((p) => (
          <article
            ...
            data-principle="true"
            data-hover="true"
            className="glass group flex flex-col gap-4 p-7 rounded-sm
                       cursor-default transition-all duration-500
                       hover:border-cobalt-500/30"
            style={{
          ...
          >
            {/* Number */}
            <span
              className="text-mono text-cobalt-500/50 text-xs tracking-[0.4em] uppercase
                         group-hover:text-cobalt-400 transition-colors duration-300"
            >
              Principle {p.number}
            </span>
            ...
            <div
              style={{
                ...
                width: '100%',
                background:
                  'linear-gradient(90deg, rgba(26,86,255,0.3), transparent)',
              }}
            />

            {/* Title */}
            <h3
              className="font-syne text-mercury-100 font-semibold"
              style={{ fontSize: 'clamp(1rem, 1.4vw, 1.15rem)', lineHeight: 1.25 }}
            >
              {p.title}
            </h3>
            ...
            {/* Body */}
            <p
              className="font-bricolage text-mercury-500 text-sm leading-relaxed
