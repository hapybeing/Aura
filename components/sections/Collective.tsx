...
export default function Collective() {
  ...
  useEffect(() => {
    ...
    // ── Section progress → Zustand → ParticleRiver uniforms ───────────────
  ...
  return (
    <section
      ...
      className="relative min-h-screen flex flex-col
                 px-6 md:px-12 lg:px-20 py-28 md:py-36 gap-20"
    >
      {/* ── SECTION HEADER ──────────────────────────────────────────────── */}
      <div className="flex flex-col gap-12">
        ...
        <div>
          <KineticText
            ...
            style={{ fontSize: 'clamp(3rem, 9vw, 9rem)' } as React.CSSProperties}
          >
            THE MINDS
          </KineticText>
          <KineticText
            ...
            style={{
              ...
              WebkitTextStroke: '1px rgba(232,232,240,0.25)',
              color: 'transparent',
              marginTop: '-0.05em',
        ...
        <KineticText
          ...
          style={{ fontSize: 'clamp(0.95rem, 1.4vw, 1.2rem)', lineHeight: 1.7 } as React.CSSProperties}
        >
      ...
      >
        {STATS.map((s, i) => (
          <div
            ...
            style={{ background: 'rgba(13,13,31,0.6)' }}
          >
            <span
        ...
        {ARTISTS.map((artist) => (
          <div key={artist.id} data-card="true">
            <GlassCard
              ...
              tiltStrength={7}
              glowColor={artist.accent}
            >
              <div className="flex flex-col gap-6 p-7 md:p-9">

                {/* Top row: index + handle */}
                <div className="flex justify-between items-start">
                  <span
                    className="text-mono text-cobalt-500/40 text-xs tracking-[0.38em]"
                  >
                    {artist.index}
                  </span>
                  <span
                    className="text-mono text-mercury-600 text-xs tracking-[0.18em]"
                  >
                    {artist.handle}
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <h3
                    className="font-syne text-mercury-100 font-bold leading-none tracking-tight"
                    style={{ fontSize: 'clamp(2rem, 4vw, 2.8rem)' }}
                  >
                    {artist.name}
                  </h3>
                  <p className="text-mono text-cobalt-400 text-xs tracking-[0.3em] uppercase">
                    {artist.role}
                  </p>
                </div>

                {/* Bio */}
                <p className="font-bricolage text-mercury-500 text-sm leading-relaxed">
                  {artist.bio}
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {artist.medium.split(' · ').map((tag) => (
                    <span
                      key={tag}
                      className="text-mono text-mercury-600 text-xs tracking-[0.2em]
                                 uppercase px-3 py-1 rounded-sm"
                      style={{
                ...
                <div className="flex flex-col gap-1.5 mt-1">
                  {artist.works.map((work) => (
                    ...
                    >
                      <div
                        className="w-1 h-1 rounded-full bg-cobalt-500/50
                                   group-hover:bg-cobalt-400 transition-colors duration-300"
                      />
                      <span
                        className="font-bricolage text-mercury-600 text-xs
                                   group-hover:text-mercury-300 transition-colors duration-300
                                   tracking-wide"
                      >
                        {work}
                      </span>
                ...
                >
                  <button
                    className="text-mono text-cobalt-400 text-xs tracking-[0.32em]
                               uppercase flex items-center gap-3 group"
                  >
                    <span
                      className="block w-5 h-px bg-cobalt-500/50
      ...
      <div className="flex justify-between items-center pt-4">
        <span className="text-mono text-mercury-600 text-xs tracking-[0.32em] uppercase">
          — 4 Members Active
        </span>
        <MagneticButton strength={22}>
          <button
            className="glass text-mono text-mercury-400 text-xs
                       tracking-[0.28em] uppercase px-6 py-3 rounded-sm
